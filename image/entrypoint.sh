...
initialize_mysql_database() {
  ...
  if [ ! -d ${MYSQL_DATA_DIR}/mysql ]; then
    ...
    while ! /usr/bin/mysqladmin -proot status >/dev/null 2>&1
    do
      timeout=$(($timeout - 1))
      if [ $timeout -eq 0 ]; then
        echo -e "\n 1: Could not connect to database server. Aborting..."
        exit 1
      fi
      echo -n "."
      sleep 1
...
create_users_and_databases() {
  ...
  if [ -n "${DB_USER}" -o -n "${DB_NAME}" ]; then
    ...
    do
      timeout=$(($timeout - 1))
      if [ $timeout -eq 0 ]; then
        echo " 2: Could not connect to mysql server. Aborting..."
        exit 1
      fi
      sleep 1
    done
